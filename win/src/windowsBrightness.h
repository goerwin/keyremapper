...
// Ensure __func__ is defined, someway or another. This is the name of the
// currently executing function.
#ifndef __func__
#define __func__ __FUNCTION__
#endif

// These macros make it so that you can just type w(SOME_RANDOM_POUND_DEFINE) to
// turn a #define'd char string into a wchar_t string.
#define __L(x) L##x
#define w(x) __L(x)
...
#define PRINT_BOOL(val) \
  printf("%s = %s\n", STRINGIFY(val), val ? "true" : "false");
#define PRINT_WSTR(val) printf("%s = '%S'\n", STRINGIFY(val), val);
#define PRINT_POINT(val) \
  printf("%s = (%d, %d)\n", STRINGIFY(val), (val).x, (val).y);
#define PRINT_RECT(val)                                                    \
  printf("%s = (%d, %d, %d, %d)\n", STRINGIFY(val), (val).left, (val).top, \
         (val).right, (val).bottom);

// This marco allows you to easily declare a static string constant (wchar_t*)
// in a class header Meaning, instead of this: 		static const wchar_t
//*wImmersiveControlPanelUMID()
//		{
...
//		STATIC_WSTR(wImmersiveControlPanelUMID,
// L"windows.immersivecontrolpanel_cw5n1h2txyewy!microsoft.windows.immersivecontrolpanel");
//
// But when you do this, you need to reference it like a function. For example
// (assuming the class name is ModernHelpers):
//		wcslen(ModernHelpers::wImmersiveControlPanelUMID());
#define STATIC_WSTR(name, text)           \
  inline static const wchar_t *(name)() { \
    static const wchar_t *str = (text);   \
    return str;                           \
  }
#define STATIC_DOUBLE(name, val) \
  inline static double(name)() { return (val); }
#define STATIC_POINT(name, x, y) \
  inline static POINT(name)() { return {(x), (y)}; }

// Prints the error specified by @error. If @error is set to ERROR_SUCCESS, it
// will be populated with GetLastError().
#define PRINT_ERROR(error)                                             \
  {                                                                    \
    DWORD err = (error);                                               \
    if (err == ERROR_SUCCESS) err = GetLastError();                    \
    wchar_t wErrorText[MAX_PATH];                                      \
    ::FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, 0, err, 0, wErrorText, \
                    MAX_PATH, 0);                                      \
...
#define FAIL_IF(statement)                                               \
  if ((statement)) {                                                     \
    printf("%s: failed because the following was true '%s'\n", __func__, \
           STRINGIFY((statement)));                                      \
    PRINT_ERROR_SHORT(ERROR_SUCCESS);                                    \
    return false;                                                        \
  }
#define CONTINUE_IF(statement)                                            \
  if ((statement) == false) {                                             \
    printf("%s: failed because the following was false '%s'\n", __func__, \
           STRINGIFY((statement)));                                       \
    PRINT_ERROR_SHORT(ERROR_SUCCESS);                                     \
    return false;                                                         \
  }
...
// Shows the slider. Returns true on success. Note: this has no effect on screen
// brightness. This is done statically at the cost of perf.
bool static ShowSlider() {
  HWND hwnd = FindWindow(L"NativeHWNDHost", L"");
  CONTINUE_IF(hwnd != NULL);
  CONTINUE_IF(
      PostMessage(hwnd, RegisterWindowMessage(L"SHELLHOOK"), 0x0037, 0));
  // CONTINUE_IF(ShowWindow(hwnd, SW_SHOW));
  return true;
}

// Gets the AC and DC brightness indexes for the current power configuration.
// Returns @DCIndex or @ACIndex, depending on power status.
DWORD static GetBrightness() {
  ...
  if (powerStatus.ACLineStatus) return ACIndex;
...
bool static SetBrightness(int val) {
  ...
  FAIL_IF(PowerWriteACValueIndex(NULL, pPwrGUID, &subGUID, &briGUID, val) !=
          ERROR_SUCCESS);
  FAIL_IF(PowerWriteDCValueIndex(NULL, pPwrGUID, &subGUID, &briGUID, val) !=
          ERROR_SUCCESS);
  // if (PowerApplySettingChanges != NULL) PowerApplySettingChanges(&subGUID,
  // &briGUID);
...
bool static Increment(int increment) {
  ...
  if (increment == 0 || Index == 100 && increment >= 0 ||
      Index == 0 && increment <= 0)
    return true;
