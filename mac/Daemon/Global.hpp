...
// cmd, cmdR, shift, shiftR, alt, altR, ctrl,
// ctrlR, fn
...
bool isArrowKeyVkCode(ushort vkCode) {
  return std::find(arrowKeyVkCodes.begin(), arrowKeyVkCodes.end(), vkCode) !=
         arrowKeyVkCodes.end();
}

bool isMediaVkKeyCode(ushort vkCode) {
  return std::find(mediaVkCodes.begin(), mediaVkCodes.end(), vkCode) !=
         mediaVkCodes.end();
}

bool isFunctionKeyVkCode(ushort vkCode) {
  return std::find(fnKeyVkCodes.begin(), fnKeyVkCodes.end(), vkCode) !=
         fnKeyVkCodes.end();
}

bool isModifierKeyVkCode(ushort vkCode) {
  return std::find(modifierVkCodes.begin(), modifierVkCodes.end(), vkCode) !=
         modifierVkCodes.end();
}
