...
class MouseManager {
 ...
 public:
  static void handleMouseDownUp(bool isMouseDown,
                                std::string mouseType = "left") {
    ...
    if (isMouseDown) {
      ...
      if (MouseManager::mouseType != mouseType ||
          newMouseLoc.x != MouseManager::mouseLoc.x ||
          newMouseLoc.y != MouseManager::mouseLoc.y || clickCount == 0 ||
          now - lastPressTime > doubleClickSpeed)
        clickCount = 1;
      else
        clickCount = clickCount + 1;

      // NOTE: When assigning to a static
      // variable, ensure you prefix the ClassName
    ...
    auto eventType = isMouseDown ? mouseType == "right" ? kCGEventRightMouseDown
                                                        : kCGEventLeftMouseDown
                     : mouseType == "right" ? kCGEventRightMouseUp
                                            : kCGEventLeftMouseUp;
    auto buttonType =
        mouseType == "right" ? kCGMouseButtonRight : kCGMouseButtonLeft;
    CGEventRef event =
        CGEventCreateMouseEvent(NULL, eventType, newMouseLoc, buttonType);
  ...
  static void setModifierFlagsToNativeMouseEvent(CGEventRef event,
    ...
    bool isDragging = eventType == kCGEventLeftMouseDragged ||
                      eventType == kCGEventRightMouseDragged;

    if (!isMoving && !isDragging) return CGEventSetFlags(event, flags);

    // if it's moving or dragging, convert event
    // to drag if mouse_is_down
    if (MouseManager::status == "mouse_is_down") {
      CGEventSetType(event, MouseManager::mouseType == "left"
  ...
  static int start() {
    // apparently, real mouse events don't
    // register modifier presses generated by this
    // app. So I have to listen for mouse events
    // and attach the modifier flags accordingly
    // NOTE: kCGEventTapOptionListenOnly does not
    // fail when clicking the app's menu bar but
    // it doesnt let me modify the event
    MouseManager::myEventTap = CGEventTapCreate(
        ...
        [](CGEventTapProxy proxy, CGEventType type, CGEventRef event,
          ...
          // only attach flags to native mouse
          // events
    ...
    MouseManager::myRunLoopSource = CFMachPortCreateRunLoopSource(
        kCFAllocatorDefault, MouseManager::myEventTap, 0);

    // Couldn't create runLoopSource
    if (!MouseManager::myRunLoopSource) return 2;

    // NOTE: kCFRunLoopCommonModes removes the
    // issue of clicking the app's menubar
    // kCFRunLoopDefaultMode has issue with
    // clicking the app's menubar Seems that on
    // xcode's menubar doesnt work when running it
    // from it :(
    CFRunLoopAddSource(CFRunLoopGetMain(), MouseManager::myRunLoopSource,
  ...
  static void stop() {
    ...
    if (MouseManager::myRunLoopSource) {
      CFRunLoopRemoveSource(CFRunLoopGetMain(), MouseManager::myRunLoopSource,
                            kCFRunLoopCommonModes);
